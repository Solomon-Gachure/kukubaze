...
const Drinks = () => {
  ...
  return (
    <div name="drinks" className="font-mono p-2 bg-orange-600/30">
      <h1 className="text-2xl lg:text-3xl font-bold text-orange-500 flex justify-center">
        Quench Your Thirst :{" "}
      </h1>
      <div className="grid md:grid-cols-2 gap-4 text-md pt-4 w-full mb-3">
        {drinks.map((drink) => (
          <div
            key={drink.idDrink}
            className="grid grid-cols-2 gap-3 md:gap-4 border border-white/30 bg-white/60 rounded-lg"
          >
            ...
            <div className="flex flex-col justify-between rounded-lg">
              <div className="gap-1 flex flex-col pt-4">
                ...
                <div>
                  <button
                    onClick={() => handleOpen(drink.idDrink)}
                    className="p-1"
                  >
                    <span className="flex flex-row lg:items-center lg:gap-1">
                      How we craft it
                      <LiaCocktailSolid className="text-orange-500" size={20} />
                    </span>
                  </button>
                </div>
                {selectedDrinkId === drink.idDrink && (
                  <div>
                    <div className="flex flex-col justify-center items-center top-0 right-0 fixed gap-2 w-full h-full z-50 bg-stone-950 bg-opacity-60 backdrop-blur-md backdrop-filter:blur(8px) bg-clip-padding text-white font-bold">
                      <p className="text-center text-yellow-300 text-3xl md:text-3xl font-dancing font-extrabold">
